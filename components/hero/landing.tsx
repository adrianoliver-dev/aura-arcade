...
import { createWorld, daySeed, MATCH_MS } from '@/lib/humo/sim'
...
export function HeroLanding() {
  ...
  return (
    <main className="relative flex h-full flex-col bg-[#0D1210]">
      <img
        src="/humo/hero-map-diorama.svg"
        alt=""
        className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-90"
      />
      <canvas ref={canvasRef} className="absolute inset-0 h-full w-full opacity-70" aria-hidden />
      <div className="relative z-10 flex h-full flex-col justify-end px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <p className="mt-3 max-w-md text-lg text-[#F4E7CF]/85">
          Hay fuego. Hay un predio. Llevá la respuesta desde la base.
        </p>
        <a
          href="/jugar"
        ...
        <p className="mt-3 text-sm text-[#C99052]">{Math.round(MATCH_MS / 1000)} segundos · un pulgar · ranking de hoy</p>
      </div>
    </main>
  )
}
